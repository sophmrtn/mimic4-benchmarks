...
import utils.mimic4csv as m4c
from utils.util import dataframe_from_csv

parser = argparse.ArgumentParser(description='Extract per-subject data from MIMIC-IV CSV files.')
parser.add_argument('mimic4_path', type=str, help='Directory containing MIMIC-IV HOSP CSV files.')
parser.add_argument('--mimic4_ed_path', '-ed', type=str, help='Directory containing MIMIC-IV ED CSV files.', required=True)
parser.add_argument('--output_path', '-o', type=str, help='Directory where per-subject data should be written.', required=True)
parser.add_argument('--event_tables', '-e', type=str, nargs='+', help='Tables from which to read events. Can be any from: labevents, vitalsign',
                    default=['labevents'])
...
parser.add_argument('--keep_items', '-i', type=str, help='CSV containing list of ITEMIDs to keep from labevents.')
parser.add_argument('--verbose', '-v', dest='verbose', action='store_true', default=True, help='Verbosity in output')
...
stays = m4c.merge_on_subject_admission(admits, stays) # order matters here ad stays has hadm_id as floats so merge with hadm_id instead
...
stays = m4c.add_omr_variable_to_stays(stays, args.mimic4_path, 'weight', tolerance=365) # using tolerance to find value 

stays = m4c.add_inhospital_mortality_to_stays(stays) # will use hospital_expire_flag if exists
...
diagnoses = m4c.convert_icd9_to_icd10(diagnoses, keep_original=False) # convert to ICD10 
diagnoses = m4c.filter_diagnoses_on_stays(diagnoses, stays, by_col='hadm_id') # use hadm_id to filter diagnoses by selected stays

ed_diagnoses = m4c.read_ed_icd_diagnoses_table(args.mimic4_ed_path)
ed_diagnoses = m4c.convert_icd9_to_icd10(ed_diagnoses, keep_original=False) # convert to ICD10 
ed_diagnoses = m4c.filter_diagnoses_on_stays(ed_diagnoses, stays)  # use stay_id (default) to filter diagnoses by selected stays

# merge into one diagnoses table on both hadm_id and stay_id
diagnoses = m4c.merge_on_subject_stay_admission(diagnoses, ed_diagnoses, suffixes=(None, '_ed'))
...
items_to_keep = set(
    [int(itemid) for itemid in dataframe_from_csv(args.keep_items)['itemid'].unique()]) if args.keep_items else None
for table in args.event_tables:
    ...
    m4c.read_events_table_and_break_up_by_subject(table_path, table, args.output_path, items_to_keep=items_to_keep,
                                              subjects_to_keep=subjects, mimic4_path=args.mimic4_path)
