...
def main():
    ...
    no_edstay = 0                 # stay_id is empty in events.csv. We try to fix such events.
    ...
    edstay_missing_in_stays = 0   # stay_id does not appear in stays.csv. We exclude such events.
    ...
    for subject in tqdm(subjects, desc='Iterating over subjects'):
        ...
        # since admissions with multiple ed stays were excluded
        ...
        cur_no_edstay = merged_df['stay_id'].isnull().sum()
        no_edstay += cur_no_edstay
        merged_df.loc[:, 'stay_id'] = merged_df['stay_id'].fillna(merged_df['stay_id_r'])
        recovered += cur_no_edstay - merged_df['stay_id'].isnull().sum()
        ...
        edstay_missing_in_stays += (merged_df['stay_id'] != merged_df['stay_id_r']).sum()
    ...
    print(f'no_edstay: {no_edstay}')
    print(f'recovered: {recovered}')
    print(f'could_not_recover: {could_not_recover}')
    print(f'edstay_missing_in_stays: {edstay_missing_in_stays}')
