...
def transform_marital(marital_series):
    ...
    return {'marital_status': marital_series.fillna('').apply(lambda m: m_map[m] if m in m_map else m_map[''])}

def transform_insurance(insurance_series):
    i_map = {'Medicare': 1, 'Medicaid': 2, 'Other': 3, '': 0} # TODO: 0 or nan?
    return {'insurance': insurance_series.fillna('').apply(lambda i: i_map[i] if i in i_map else i_map[''])}
...
def assemble_episodic_data(stays, diagnoses=None):
    data = {'hadm_id': stays.hadm_id, # originally 'stay' was 'stay_id'
    ...
    data = data[['race', 'gender', 'age', 'insurance', 'marital_status', 'height', 'weight', 'los', 'mortality', 'stay_id']]
    # not using diagnoses
    if diagnoses is not None:
        return data.merge(extract_diagnosis_labels(diagnoses), left_index=True, right_index=True).set_index('stay_id')
    else:
...
def extract_diagnosis_labels(diagnoses):
    ...
    diagnosis_labels = [mapper.map(x, source='icd9', target='icd10') for x in diagnosis_labels]

    labels = diagnoses[['hadm_id', 'icd_code', 'value']].drop_duplicates()\
                      .pivot(index='hadm_id', columns='icd_code', values='value').fillna(0).astype(int)
    ...
    missing_labels = DataFrame(columns=[label for label in diagnosis_labels if label not in labels.icd_code.unique()])
...
def clean_lab(df):
    v = df['value'].copy()
    idx = v.apply(lambda s: isinstance(s,str) and not re.match('^(\d+(\.\d*)?|\.\d+)$', s))
    v.loc[idx] = np.nan
    return v.astype(float)
...
def clean_temperature(df):
    ...
    idx = df['valueuom'].fillna('').apply(lambda s: 'F' in s.lower()) | df['label'].apply(lambda s: 'F' in s.lower()) | (v >= 79)  # noqa: PLR2004
