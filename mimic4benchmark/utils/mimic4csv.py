...
def read_patients_table(mimic4_path):
    ...
    pats = pats[['subject_id', 'gender', 'anchor_age', 'anchor_year', 'dod']] #dod for out of hospital mortality
...
def read_admissions_table(mimic4_path):
    ...
    admits = admits[['subject_id', 'hadm_id', 'admittime', 'dischtime', 'deathtime', 'insurance', 'marital_status', 'race', 'hospital_expire_flag']]
    admits.admittime = pd.to_datetime(admits.admittime)
    admits.dischtime = pd.to_datetime(admits.dischtime)
    admits.deathtime = pd.to_datetime(admits.deathtime)
    # add los column (in fractional days)
    diff = admits.dischtime - admits.admittime
    admits['los'] = diff.apply(lambda x: x.total_seconds() / datetime.timedelta(days=1).total_seconds()) # units: fractional day
    return admits
...
def read_icd_diagnoses_table(mimic4_path):
    ...
    diagnoses = util.dataframe_from_csv(os.path.join(mimic4_path, 'diagnoses_icd.csv.gz'))
...
def convert_icd9_to_icd10(diagnoses_df, keep_original=True):
    ...
    diagnoses_df.loc[diagnoses_df.icd_version==ICD9, 'icd_code'] = diagnoses_df.loc[diagnoses_df.icd_version==ICD9, 'icd_code'].apply(lambda x: mapper.map(x, source='icd9', target='icd10'))
...
def read_vitalsign(table):
    vitalsign_column_map = {'temperature': 'Temperature', 
    ...
    table= util.melt_table(table, id_vars=['subject_id', 'stay_id', 'charttime'], 
                           value_vars=['Temperature', 'Heart rate', 'Respiratory rate', 'Oxygen saturation', 'Systolic blood pressure', 'Diastolic blood pressure'])
    ...
    vitalsign_uom_map = {'Temperature': '°F', 
                            'Heart rate': 'bpm', 
...
def merge_on_subject_stay_admission(table1, table2, suffixes=('_x', '_y')):
    return table1.merge(table2, how='inner', left_on=['subject_id', 'hadm_id', 'stay_id'], right_on=['subject_id', 'hadm_id', 'stay_id'], suffixes=suffixes)


def add_age_to_stays(stays):
    stays['age'] = stays.apply(lambda e: ((e['admittime'].to_pydatetime().year - e['anchor_year']) // 3 + e['anchor_age']),
                               axis=1) # increases age every 3 years based on time elapsed between admission year and birth year 
...
def add_omr_variable_to_stays(stays, mimic4_path, variable, tolerance=None):
    ...
    def get_value_from_omr(omr, row, result_names):
        ...
        omr['diff'] = omr['chartdate'].apply(lambda x: np.abs((date-x.date()).days)).astype(int)
    ...
    stays[variable.lower()] = stays.apply(lambda r: get_value_from_omr(omr, r, omr_names), axis=1)
    return stays
...
def filter_diagnoses_on_stays(diagnoses, stays, by_col='stay_id'):
    return diagnoses.merge(stays[['subject_id', 'hadm_id', 'stay_id']].drop_duplicates(), how='inner',
                           left_on=['subject_id', by_col], right_on=['subject_id', by_col])
...
def read_events_table_by_row(table, nb_row=1000):
    for i, row in table.iterrows():
        if 'hadm_id' not in row:
            row['hadm_id'] = np.nan # ensure hadm_id column is present (e.g., missing for vitalsign data)
        yield row, i, nb_row

def read_events_table_and_break_up_by_subject(table_path, table, output_path,
                                              items_to_keep=None, subjects_to_keep=None, mimic4_path=None):
    
    obs_header = ['subject_id', 'hadm_id', 'stay_id', 'charttime', 'itemid', 'value', 'valueuom', 'label']
    ...
    for row, _, _ in tqdm(read_events_table_by_row(table_df), 
                          total=len(table_df),
        ...
        row_out = {'subject_id': row['subject_id'],
                   'hadm_id': '' if ('hadm_id' not in row) or (np.isnan(row['hadm_id'])) else int(row['hadm_id']), # labevents stored hadm_id's as floats so converts, and if missing (vitalsign) or nan (some labevents) then input as '' 
