...
def read_events(subject_path, remove_null=True):
    ...
    events.hadm_id = events.apply(lambda row: impute_missing_id(row.stay_id, stays) if np.isnan(row.hadm_id) else row.hadm_id, axis=1).astype(int) 
    events.stay_id = events.apply(lambda row: impute_missing_id(row.hadm_id, stays, impute_col='stay_id') if ('stay_id' not in row) or np.isnan(row.stay_id) else row.stay_id, axis=1).astype(int) 
...
def convert_events_to_timeseries(events, variable_column='itemid', variables=None):
    if variables is None:
        variables = []
    
    metadata = events[['charttime', 'hadm_id', 'stay_id']].sort_values(by=['charttime', 'hadm_id', 'stay_id'])\
