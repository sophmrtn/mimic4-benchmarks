...
def get_events_for_admission(events, hadm_id, intime=None, outtime=None):
    idx = (events.stay_id == hadm_id)
    if intime is not None and outtime is not None:
        idx = idx | ((events.charttime >= intime) & (events.charttime <= outtime))
    events = events[idx]
    del events['hadm_id']
...
def convert_events_to_timeseries(events, variable_column='variable', variables=None):
